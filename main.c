...
static uint32_t MEMORY_READ(uint32_t ADDRESS, uint32_t SIZE)
{
    ...
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_RESERVED, "ATTEMPT TO READ FROM RESERVED ADDRESS RANGE: 0x%08X", ADDRESS);
        goto MALFORMED_READ;
    }

    if(ADDRESS > M68K_MAX_MEMORY_SIZE)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BOUNDS, "ATTEMPT TO READ FROM AN ADDRESS RANGE BEYOND THE ADDRESSABLE SPACE: 0x%08X", ADDRESS);
        goto MALFORMED_READ;
    }
    ...
    MEM_ERROR(MEM_ERR, MEM_ERR_UNMAPPED, "NO BUFFER FOUND FOR ADDRESS: 0x%08X", ADDRESS);
...
static void MEMORY_WRITE(uint32_t ADDRESS, uint32_t SIZE, uint32_t VALUE)
{
    ...
    if(ADDRESS < M68K_MAX_ADDR_START || ADDRESS > M68K_MAX_ADDR_END)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_RESERVED, "ATTEMPT TO WRITE TO RESERVED ADDRESS RANGE: 0x%08X", ADDRESS);
        goto MALFORMED_WRITE;
    }

    if(ADDRESS > M68K_MAX_MEMORY_SIZE)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BOUNDS, "ATTEMPT TO WRITE TO AN ADDRESS RANGE BEYOND THE ADDRESSABLE SPACE: 0x%08X", ADDRESS);
        goto MALFORMED_WRITE;
    }
    ...
    if(MEM_BASE != NULL)
    {
        ...
        {
            MEM_BASE->USAGE.VIOLATION++;
            MEM_ERROR(MEM_ERR, MEM_ERR_READONLY, "WRITE ATTEMPT TO READ-ONLY MEMORY AT 0x%08x, VIOLATION #%u", ADDRESS, MEM_BASE->USAGE.VIOLATION);
            goto MALFORMED_WRITE;
        }
...
static void MEMORY_MAP(uint32_t BASE, uint32_t END, bool WRITABLE) 
{
    ...
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BUFER, "CANNOT MAP - TOO MANY BUFFERS");
        return;
    }

    if(END > M68K_MAX_MEMORY_SIZE)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BUS, "END ADDRESS 0x%08X EXCEEDS THE BUS LIMIT: (%d%s)", END, FORMAT_SIZE(END), FORMAT_UNIT(END));
        return;
    }
...
int main(void) 
{
    ...
    MEMORY_MAP(0x400000, 0x4FFFFF, false);
    ...
    uint32_t UNMAPPED = M68K_READ_MEMORY_32(0x00300000);
    uint32_t RO_READ = M68K_READ_MEMORY_32(0x400000);
    M68K_WRITE_MEMORY_32(0x0007FFFF, 0x12345678);
    M68K_WRITE_MEMORY_16(0x0080FFFF, 0xABCD);
    M68K_READ_MEMORY_8(0x01000000);
    M68K_READ_MEMORY_16(0x00FFFFFF);
    M68K_WRITE_MEMORY_8(0x00FFFFFF, 0xFF);
