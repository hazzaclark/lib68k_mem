...
static void BUS_ERROR(M68K_BERR_TYPE TYPE, uint32_t ADDRESS, M68K_MEM_OP MEM_OP, uint32_t SIZE)
{
    ...
    if(BERR_STATE.ACTIVE && !BERR_STATE.HALT_LINE)
    {
        ...
        MEM_ERROR(MEM_ERR, MEM_ERR_BERR, SIZE, "ORIGINAL FAULT AT 0x%08X\n", BERR_STATE.CURRENT_ADDRESS);
        MEM_ERROR(MEM_ERR, MEM_ERR_BERR, SIZE, "NEW FAULT AT 0x%08X\n", ADDRESS);
...
static uint32_t MEMORY_READ(uint32_t ADDRESS, uint32_t SIZE)
{
    ...
    if(MEM_BASE != NULL)
    {
        ...
        {
            MEM_BASE->USAGE.BUS_ERROR++;
            MEM_ERROR(MEM_ERR, MEM_ERR_BERR, SIZE, "BERR ACTIVE FOR CURRENT BUFFER: %u", MEM_BASE->BUFFER);
            goto MALFORMED_READ;
        }
